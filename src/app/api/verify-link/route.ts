...
const verifyForgetPassoword = async (token: string): Promise<[boolean, string]> => {
  ...
  if (expiryDate < todaysDate) {
    ...
    const emailData: EmailData = {
      ...
      html: `Click the following link for reset your password.
                <br> 
                <a href="http://localhost:3000/verify-link-page?token=${newToken}&type=verifyEmail"> 
                  Click Me 
                </a>
                `,
    };
    ...
    const expiryDuration = parseInt(process.env.FORGET_PASSWORD_TOKEN_EXPIRY_HOUR!);
    const expiryDateTime = moment().utc().add(expiryDuration, "hours").toDate();
    await UserDb.updateOne(
      { email: user.email },
